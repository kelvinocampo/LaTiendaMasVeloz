...
BEGIN
    ...
    INSERT INTO inputBill (supplierID, creationDate, workerID)
    VALUES (_entityID, _creationDate, _workerID);

    SET new_bill_id = LAST_INSERT_ID(); -- Obtiene el ID de la última inserción

    SELECT new_bill_id AS ID; -- Devuelve el ID
END //
DELIMITER ;

DROP PROCEDURE IF EXISTS CrearDetalleFactura; 
DELIMITER // 
CREATE PROCEDURE CrearDetalleFactura( IN _type ENUM("IN","OUT"), IN _billID INT, IN _productID INT, IN _quantity INT, IN _price INT)
BEGIN 

    IF _type = 'IN' THEN
        INSERT INTO detailsInputBill(billID, productID, quantity, price)
        VALUES (_billID, _productID, _quantity, _price);
    ELSEIF _type = 'OUT' THEN
        INSERT INTO detailsOutputBill(billID, productID, quantity, price)
        VALUES (_billID, _productID, _quantity, _price);
    END IF;
